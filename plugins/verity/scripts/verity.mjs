...
/**
 * THE FIRST SESSION IN AN UNSET-UP PROJECT REVIEWS NOTHING, AND SAYS SO.
 *
 * `verity baseline capture`, `intent capture`, `compact` and `session end` all
 * stand down when the project has no `.verity/` — deliberately, so Verity does
 * not scatter state through every directory someone opens Claude Code in.
 * `verity analyze` does not: it runs, creates `.verity/`, and then, with no
 * baseline to compare against, classifies the turn `authored=no` and sends
 * nothing.
 *
 * On the npm path that gap never opens, because `verity init` runs first. The
 * plugin has no such step — install is one command and the next thing that
 * happens is a session — so the first session looks like a working gate and is
 * not one. Measured in a sandbox install: the agent created a file, and the
 * review reported `sent 0 · not sent hello.js`.
 *
 * One line, on SessionStart only, to stderr. Not stdout: SessionStart stdout
 * becomes agent context, and this is for the person.
 */
function nudgeIfUnconfigured() {
  try {
    let dir = process.cwd()
    for (let i = 0; i < 40; i++) {
      if (existsSync(join(dir, '.verity'))) return
    ...
    warn(
      'this project is not set up yet — the gate will not review anything until it is. ' +
        'Run /verity:setup in Claude Code.',
    )
  } catch {
    // A nudge is never worth failing a hook over.
  }
}
...
async function main() {
  ...
  // SessionStart is the one hook that fires exactly once per session, so it is
  // where a once-per-session message belongs.
  if (args[0] === 'baseline') nudgeIfUnconfigured()
  ...
  warn(
    'Verity CLI not found — this session is running without the quality gate. ' +
      'Try `/plugin update verity@codacy`, or install it directly with ' +
