 ...
 *
 * ⚠ NO userConfig, DELIBERATELY. Declaring one — even entirely optional — makes
 * Claude Code show a "Configure Verity" screen during install, so the very first
 * thing a new user is asked for is a backend URL they do not have and must not
 * need. Almost nobody is on a self-hosted or pre-release stack, and the ones who
 * are can say so afterwards, out of the way of everyone else:
 * `verity config service-url <url>`, or VERITY_SERVICE_URL in the environment,
 * which this process inherits like any other.
 */
...
async function main() {
  ...
  // ⚠ ONE SPELLING OF THE UPDATE COMMAND, shared with the CLI's own remedy in
  // cli/src/lib/cli-provenance.ts (PLUGIN_UPDATE_REMEDY). This file cannot import
  // it — it is the standalone launcher that runs BEFORE any CLI is found — so
  // scripts/check-plugin-commands.mjs fails the build if the two drift apart or
  // if anything reintroduces the `/plugin update` slash command, which does not
  // exist (VRT-210, re-checked against the Claude Code docs for VRT-240).
  warn(
    'Verity CLI not found — this session is running without the quality gate. ' +
      'Start a new Claude Code session so the plugin can reinstall it, or repair the install with ' +
      '`claude plugin update verity@verity, then /reload-plugins (or restart Claude Code)`. ' +
      `You can also install the CLI directly: \`npm install -g ${CLI_PACKAGE}\`.`,
  )
