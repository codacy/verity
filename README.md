...
Then, in your project: **`/verity:setup`**
...
so it can offer `/verity:setup`. A project that is **set up but not logged in** runs the
...
in `/plugin` → **Marketplaces** → `codacy` → **Enable auto-update**, or update when
you feel like it:
